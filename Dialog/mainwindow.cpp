    ...
    , ui(new Ui::MainWindow)
{
   ...
   QObject::connect(ui->tabWidget, &QTabWidget::currentChanged, this, &MainWindow::change_tab_widget);
   ...
   QObject::connect(thread_manager_.get(), &ThreadManager::send_work_status_to_optial_tool_dlg, this,&MainWindow::recv_work_status_form_thread_manager);
...
void MainWindow::change_tab_widget(int index) {
    if (0 != index) {
...
void MainWindow::setLabelColor(QLabel* label, const QColor& color) {
...
void MainWindow::recv_work_status_form_thread_manager(int widget_index, bool status){
    if(widget_index == 1){
        if(status == true)
...
void MainWindow::setHalfScreenSize() {
